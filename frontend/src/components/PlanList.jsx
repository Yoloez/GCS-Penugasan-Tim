                ...
                {plans.map((plan) => (
                  <div
                    ...
                    className={`bg-white/10 backdrop-blur-lg hover:bg-white/20 cursor-pointer p-4 rounded-2xl border transition-all duration-200 ${
                      activePlan?.id === plan.id ? "border-blue-400 border-2 shadow-lg shadow-blue-500/20" : "border-white/20 hover:border-white/30"
                    }`}
                    onClick={() => handlePlanClick(plan)}
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        <strong className="text-white text-base block truncate">{plan.name}</strong>
                      ...
                      <div className="flex flex-col gap-2 ml-3">
                        ...
                        <button onClick={(e) => handleExportClick(plan, e)} className="bg-white/10 hover:bg-green-500/80 border-none text-white cursor-pointer text-sm rounded-lg p-2 transition-all hover:scale-110" title="Export">
                          ⬇️
                        </button>
                        <button onClick={(e) => handleDeleteClick(plan, e)} className="bg-white/10 hover:bg-red-500/80 border-none text-white cursor-pointer text-sm rounded-lg p-2 transition-all hover:scale-110" title="Delete">
                          🗑️
                        </button>
                      </div>
                    </div>
      ...
      {editingPlan && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-2000">
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-6 min-w-[400px] shadow-2xl border border-white/20">
            <h2 className="mt-0 mb-4 text-blue-600 text-xl font-semibold">Edit Plan</h2>
            ...
            <div className="bg-blue-50 p-3 rounded-lg mb-4 text-sm text-gray-700">
              <strong>Note:</strong> To edit waypoints/geometry, close this dialog and use the 🗺️ button.
            </div>
