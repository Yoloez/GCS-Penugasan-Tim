...
const MapEvents = ({ drawingMode, onPlanSave, onDrawingCancel }) => {
	...
	useMapEvent({
		mousemove(e) {
			...
			if (drawingMode) {
				mapElement.style.cursor = "crosshair !important";
				mapElement.classList.add("drawing-cursor");
			} else {
				mapElement.style.cursor = "grab !important";
				mapElement.classList.remove("drawing-cursor");
			}
		},
		mouseenter(e) {
			const mapElement = e.target.getContainer();
			...
			switch (drawingMode) {
				...
				case "circle":
					if (points.length === 0) {
						...
						const radiusInDegrees = Math.sqrt(
							Math.pow(lat - centerLat, 2) + Math.pow(lng - centerLng, 2)
						);
						...
						for (let i = 0; i < numPoints; i++) {
							...
							const perimeterLat =
								centerLat + radiusInDegrees * Math.cos(angle);
							const perimeterLng =
								centerLng + radiusInDegrees * Math.sin(angle);
		...
		onEnter: () => {
			...
			if (points.length > 0 && onPlanSave) {
				...
				if (drawingMode === "rectangle" && points.length === 2) {
					...
					const radiusInDegrees = Math.sqrt(
						Math.pow(edgeLat - centerLat, 2) + Math.pow(edgeLng - centerLng, 2)
					);
