...
import { createPlan, getPlans, deletePlan as apiDeletePlan, updatePlan } from "../../services/api";
...
const PlanningPageEnhanced = () => {
    ...
    try {
      ...
      if (editingPlanId) {
        ...
        const updatedData = {
          ...
          description: existingPlan.description || `Type: ${pendingPlanData.type}`,
      ...
      setError(editingPlanId ? "Failed to update plan" : "Failed to save plan to database");
      alert(editingPlanId ? "❌ Failed to update plan geometry. Please try again." : "❌ Failed to save plan to database. Please check if backend is running.");
    } finally {
      setIsLoading(false);
    }
  ...
  const handlePlanEditGeometry = useCallback((plan) => {
    ...
    alert(`✏️ Edit mode activated for "${plan.name}".\n\nDraw the new geometry on the map. The old waypoints will be replaced.`);
  }, []);

  return (
    <div className="planning-container">
      <MapPlanning mapType={mapType} drawingMode={drawingMode} activePlan={activePlan} onPlanSave={handlePlanSave} onDrawingCancel={handleDrawingCancel} onMapInit={setMapRef} />
      ...
      {editingPlanId && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-2001 bg-purple-600/90 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3">
          <span>
            🗺️ Editing Geometry: <strong>{plans.find((p) => p.id === editingPlanId)?.name}</strong>
          </span>
          <button
            onClick={() => {
      ...
      {showNameDialog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-2000">
          <div className="bg-linear-to-r from-slate-700 via-gray-700 to-slate-800 rounded-xl shadow-2xl text-center w-full max-w-md mx-4 p-6 animate-in">
            <h2 className="text-2xl font-bold text-white mb-6 text-center">{editingPlanId ? "Confirm Geometry Update" : "Name Your Plan"}</h2>

            {editingPlanId ? (
              <div className="mb-6 text-center">
                <p className="text-white mb-2 text-lg text-center">
                  Update geometry for <strong className="text-white uppercase text-center">{plans.find((p) => p.id === editingPlanId)?.name}</strong>?
                </p>
                <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded p-4 text-sm text-gray-700">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-left">
                      ...
                      <span>This will replace the old waypoints with the new geometry you just drew.</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={handleCancelNameDialog} className="px-6 py-2.5 rounded-lg bg-white/10 hover:bg-red-500/80 text-white cursor-pointer text-sm font-semibold transition-all shadow-md hover:shadow-white/20">
                Cancel
              </button>
              <button onClick={handleConfirmPlanName} className="px-6 py-2.5 rounded-lg bg-white/10 text-white cursor-pointer text-sm font-semibold hover:bg-green-500/80 transition-all shadow-md hover:shadow-white/20">
      ...
      <PlanList plans={plans} activePlan={activePlan} onPlanLoad={handlePlanLoad} onPlanDelete={handlePlanDelete} onPlanUpdate={handlePlanUpdate} onPlanEditGeometry={handlePlanEditGeometry} />
    </div>
  );
};
