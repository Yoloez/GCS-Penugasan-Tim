      ...
      {showNameDialog && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-2000">
          <div className="bg-white rounded-lg p-8 min-w-[300px] shadow-2xl">
            <h2 className="mt-0 mb-4 text-blue-600 text-xl font-semibold">{editingPlanId ? "Confirm Geometry Update" : "Name Your Plan"}</h2>

            {editingPlanId ? (
              <div className="mb-6">
                <p className="text-gray-700 mb-2">
                  Update geometry for <strong>{plans.find((p) => p.id === editingPlanId)?.name}</strong>?
                </p>
                <div className="bg-yellow-50 border border-yellow-200 rounded p-3 text-sm text-gray-700">
                  ⚠️ <strong>Warning:</strong> This will replace the old waypoints with the new geometry you just drew.
                </div>
              </div>
            ) : (
              <input
